...
/*
ngModal: <ng-modal>...</ng-modal>
  - Modal directive
  - Available attributes:
    ~ bgColor, sets opaque background color, default black
    ~ ngOpacity, sets opacity of background, default 0.7
    ~ ngWidth, sets modal width, default 40%
    ~ ngHeight, sets modal height, default 40%
    ~ 

ngPersist: ng-persist
...
ngDraggable: Make an element draggable
    dragOptions = {
...
angular.module('ngAddons', [])

  .directive('ngModal', function() {
    return {
      restrict: 'E',
      template: "<div ng-show='ngModalShow' ng-style='bgstyling()'></div>" +
                "<div ng-show='ngModalShow' ng-style='fgstyling()' ng-transclude></div>",
      transclude: true,
      scope: false,
      link: function (scope, elem, attrs) {
          var elem_ = elem[0].getElementsByTagName('div')[1].firstChild;
          var width = window.innerWidth, height = window.innerHeight;
          var ngWidth = attrs.ngWidth || '40%';
          var ngHeight = attrs.ngHeight || '40%';
          ...
          scope.fgstyling = function() {
              return {
                  'position': 'fixed',
                  'width': attrs.ngWidth || '40%',
                  'height': attrs.ngHeight || '40%',
                  'left': (100 - parseInt(ngWidth)) / 2 + '%',
                  'top': (100 - parseInt(ngHeight)) / 2 + '%'
              }
          }
      }
    }
  })
  .directive('ngPersist', function() {
